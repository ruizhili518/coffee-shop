...
const Page = ({ params }: { params: { manageById: string } }) => {
    ...
    const onSubmit = async (values: FormValues) => {
        ...
        try {
            const res = await updateProductById(params.manageById, formData);
            if(res.status === 200){
                setAlertTitle("Update product successfully")
    ...
    // Get product info and initialize the form using the product data.
    ...
    useEffect(() => {
        getInfo(params.manageById);
    }, [params.manageById]);
    useEffect(() => {
        if (productData) {
            productData.customizations.forEach(customization => {
                customization.extraprice = String(customization.extraprice);
            })
            ...
            methods.reset({
                ...
                buy: String(productData.buy) || "0",
                getFree: String(productData.getFree) || "0",
